# From https://github.com/zju3dv/NeuralRecon/tree/master/tools/tsdf_fusion
import os
import sys
...
from representations.tsdf_fusion.fusion import *
from representations.tsdf_fusion.simple_loader import *
...
def parse_args():
    ...
    parser.add_argument("--max_depth", default=3.0, type=float, help="mask out large depth values since they are noisy")
...
# python representations/tsdf_fusion/generate_gt.py --dataset scannet --data_path data/ --save_name scans/scene0000_00/tsdf --n_gpu 1 --num_workers 1
args = parse_args()
...
def save_tsdf_full(args, scene_path, cam_intr, depth_list, cam_pose_list, color_list, save_mesh=False):
    ...
    for id in depth_list.keys():
        if id % 100 == 0:
            print(f"{scene_path}: Fusing frame {str(id)}/{str(n_imgs)}")
        depth_im = depth_list[id]
        cam_pose = cam_pose_list[id]
        if len(color_list) == 0:
            color_image = None
        ...
        for l in range(args.num_layers):
            tsdf_vol_list[l].integrate(color_image, depth_im, cam_intr, cam_pose, obs_weight=1.0)
    ...
    for l in range(args.num_layers):
        ...
        np.savez_compressed(os.path.join(args.save_path, scene_path, f"full_tsdf_layer{str(l)}"), tsdf_vol)

    if save_mesh:
        for l in range(args.num_layers):
            print(f"Saving mesh to mesh{str(l)}.ply...")
...
def save_fragment_pkl(args, scene, cam_intr, depth_list, cam_pose_list):
    fragments = []
    print(f"segment: process scene {scene}")
...
def process_with_single_worker(args, scannet_files):
    for scene in tqdm(scannet_files):
        ...
        if args.dataset == "scannet":
            ...
            # dataset = ScanNetDataset(args.data_path + "/" + scene, ray_sampling_strategy="all_images")

        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=None, collate_fn=collate_fn, batch_sampler=None, num_workers=args.loader_num_workers
        )

        for id, (cam_pose, depth_im, _) in enumerate(dataloader):
            if id % 100 == 0:
                print(f"{scene}: read frame {str(id)}/{str(n_imgs)}")
...
def generate_pkl(args):
    ...
    if not args.test:
        splits = ["train", "trainval"]
    else:
        splits = ["test"]
    for split in splits:
        fragments = []
        # with open(os.path.join(args.save_path, "splits", f"scannetv2_{split}.txt")) as f:
        with open(os.path.join(args.save_path, f"../{split}.txt")) as f:
        ...
        with open(os.path.join(args.save_path, f"fragments_{split}.pkl"), "wb") as f:
            pickle.dump(fragments, f)
